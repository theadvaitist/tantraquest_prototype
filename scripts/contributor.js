...
function loadMyContribs() {
  ...
  if (!mine.length) {
    myContribsDiv.innerHTML = '<div class="text-sand-600 dark:text-sand-400 p-4 italic">No contributions saved locally from this browser.</div>';
    return;
  }
  mine.forEach(item => {
    const el = document.createElement('div'); 
    el.className = 'flex justify-between items-center border border-sand-200 dark:border-slate-700 rounded-xl bg-sand-50 dark:bg-slate-700/30 p-4 mb-4 shadow-md';
    const left = document.createElement('div');
    left.innerHTML = `<strong class="text-terracotta-700 dark:text-terracotta-300">${item.concept || item.tantrayukti || 'Untitled'}</strong><br/><small class="text-sand-700 dark:text-sand-300 italic">${item.missingPart}</small>`;
    const right = document.createElement('div');
    const editBtn = document.createElement('button'); 
    editBtn.innerText = 'Edit';
    editBtn.className = 'bg-terracotta-600 hover:bg-terracotta-700 text-white px-3 py-1 rounded-lg font-medium transition-colors shadow-sm mr-2';
    const delBtn = document.createElement('button'); 
    delBtn.innerText = 'Delete';
    delBtn.className = 'bg-sand-700 hover:bg-sand-800 text-white px-3 py-1 rounded-lg font-medium transition-colors shadow-sm';
...
if (msgDiv) {
  msgDiv.className = 'mt-3 text-center text-terracotta-700 dark:text-terracotta-300 text-sm font-medium';
}
